...
<head>
  ...
  <title>Admin Portal | Schools</title>
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <button type="button" name="add" id="add_button" data-toggle="modal" data-target="#schoolModal" class="btn btn-warning">Add</button>   
            </ol>
          </div>
        </div>
      </div>
...
<script>
  $(function () {
    $('#add_button').click(function(){
      $('#school_form')[0].reset();
      $('.modal-title').html("<i class='fa fa-plus'></i> Add School");
      $('#action').val('Add');
      $('#btn_action').val('Add');
    });
    ...
    $(document).on('click', '.status', function(){
      ...
      if(confirm("Are you sure you want to change status?"))
      {
        $.ajax({
          url:"action.php",
          method:"POST",
          data:{school_id:school_id, status:status, btn_action:btn_action},
          success:function(data)
          {
            $('#alert_action').fadeIn().html('<div class="alert alert-warning alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><i class="icon fa fa-info"></i>'+data+'</div>');
            schooldataTable.ajax.reload();
    ...
    $(document).on('click', '.delete', function(){
      ...
      if(confirm("Are you sure you want to delete?"))
      {
        $.ajax({
          url:"action.php",
          method:"POST",
          data:{school_id:school_id, btn_action:btn_action},
          success:function(data)
          {
            $('#alert_action').fadeIn().html('<div class="alert alert-warning alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><i class="icon fa fa-info"></i>'+data+'</div>');
            schooldataTable.ajax.reload();
    ...
    var schooldataTable = $('#example1').DataTable({
